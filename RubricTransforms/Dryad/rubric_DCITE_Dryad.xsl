...
{</xsl:text><xsl:text>"title":"FGDC for Data Discovery - Mandatory",</xsl:text><xsl:text>"code":"FGDC_Mandatory",</xsl:text><xsl:text>"description":"The FGDC Content Standard for Digital Geospatial Metadata includes fields in the Identification Information and Metadata Reference Sections that are mandatory for data discovery. In the graphical representation of the standard, these are yellow fields included in yellow sections.",</xsl:text><xsl:text>"maximum":"20",</xsl:text><xsl:variable name="FGDC_Mandatory.itemExists" as="xs:integer+"><xsl:sequence select="if(exists(/dryad:*/dcterms:title)) then 1 else 0"></xsl:sequence><xsl:sequence select="if(exists(/dryad:*/dcterms:description | /dryad:*/dcterms:abstract)) then 1 else 0"></xsl:sequence><xsl:sequence select="if(exists(/dryad:*/dcterms:temporal)) then 1 else 0"></xsl:sequence><xsl:sequence select="if(exists(/dryad:*/dcterms:license)) then 1 else 0"></xsl:sequence><xsl:sequence select="if(exists(/dryad:*/dcterms:subject)) then 1 else 0"></xsl:sequence></xsl:variable><xsl:text>"</xsl:text><xsl:value-of select="'existTotal'"></xsl:value-of><xsl:text>":"</xsl:text><xsl:value-of select="sum($FGDC_Mandatory.itemExists)"></xsl:value-of><xsl:text>",
</xsl:text><xsl:variable name="FGDC_Mandatory.itemCounts" as="xs:integer+"><xsl:sequence select="count(/dryad:*/dcterms:title)"></xsl:sequence><xsl:sequence select="count(/dryad:*/dcterms:description) + count(/dryad:*/dcterms:abstract)"></xsl:sequence><xsl:sequence select="count(/dryad:*/dcterms:temporal)"></xsl:sequence><xsl:sequence select="count(/dryad:*/dcterms:license)"></xsl:sequence><xsl:sequence select="count(/dryad:*/dcterms:subject)"></xsl:sequence></xsl:variable><xsl:text>"</xsl:text><xsl:value-of select="'countTotal'"></xsl:value-of><xsl:text>":"</xsl:text><xsl:value-of select="sum($FGDC_Mandatory.itemCounts)"></xsl:value-of><xsl:text>",
</xsl:text><xsl:text>
"items" : [
</xsl:text><xsl:text>{
</xsl:text><xsl:text>"name":"Resource Title",</xsl:text><xsl:variable name="FGDC_Mandatory.itemExists.d1e286" as="xs:integer+"><xsl:sequence select="if(exists(/dryad:*/dcterms:title)) then 1 else 0"></xsl:sequence></xsl:variable><xsl:text>"</xsl:text><xsl:value-of select="'exists'"></xsl:value-of><xsl:text>":"</xsl:text><xsl:value-of select="sum($FGDC_Mandatory.itemExists.d1e286)"></xsl:value-of><xsl:text>",
</xsl:text><xsl:variable name="FGDC_Mandatory.itemCounts.d1e286" as="xs:integer+"><xsl:sequence select="count(/dryad:*/dcterms:title)"></xsl:sequence></xsl:variable><xsl:text>"</xsl:text><xsl:value-of select="'count'"></xsl:value-of><xsl:text>":"</xsl:text><xsl:value-of select="sum($FGDC_Mandatory.itemCounts.d1e286)"></xsl:value-of><xsl:text>",
</xsl:text><xsl:text>
"paths" : [
</xsl:text><xsl:text>
...
<xsl:text>{
</xsl:text><xsl:text>"name":"Abstract",</xsl:text><xsl:variable name="FGDC_Mandatory.itemExists.d1e542" as="xs:integer+"><xsl:sequence select="if(exists(/dryad:*/dcterms:description | /dryad:*/dcterms:abstract)) then 1 else 0"></xsl:sequence></xsl:variable><xsl:text>"</xsl:text><xsl:value-of select="'exists'"></xsl:value-of><xsl:text>":"</xsl:text><xsl:value-of select="sum($FGDC_Mandatory.itemExists.d1e542)"></xsl:value-of><xsl:text>",
</xsl:text><xsl:variable name="FGDC_Mandatory.itemCounts.d1e542" as="xs:integer+"><xsl:sequence select="count(/dryad:*/dcterms:description | /dryad:*/dcterms:abstract)"></xsl:sequence></xsl:variable><xsl:text>"</xsl:text><xsl:value-of select="'count'"></xsl:value-of><xsl:text>":"</xsl:text><xsl:value-of select="sum($FGDC_Mandatory.itemCounts.d1e542)"></xsl:value-of><xsl:text>",
</xsl:text><xsl:text>
"paths" : [
</xsl:text><xsl:text>
...
<xsl:text>{
</xsl:text><xsl:text>"name":"Temporal Extent",</xsl:text><xsl:variable name="FGDC_Mandatory.itemExists.d1e6465" as="xs:integer+"><xsl:sequence select="if(exists(/dryad:*/dcterms:temporal)) then 1 else 0"></xsl:sequence></xsl:variable><xsl:text>"</xsl:text><xsl:value-of select="'exists'"></xsl:value-of><xsl:text>":"</xsl:text><xsl:value-of select="sum($FGDC_Mandatory.itemExists.d1e6465)"></xsl:value-of><xsl:text>",
</xsl:text><xsl:variable name="FGDC_Mandatory.itemCounts.d1e6465" as="xs:integer+"><xsl:sequence select="count(/dryad:*/dcterms:temporal)"></xsl:sequence></xsl:variable><xsl:text>"</xsl:text><xsl:value-of select="'count'"></xsl:value-of><xsl:text>":"</xsl:text><xsl:value-of select="sum($FGDC_Mandatory.itemCounts.d1e6465)"></xsl:value-of><xsl:text>",
</xsl:text><xsl:text>
"paths" : [
</xsl:text><xsl:text>
...
<xsl:text>{
</xsl:text><xsl:text>"name":"Resource Use Constraints",</xsl:text><xsl:variable name="FGDC_Mandatory.itemExists.d1e5063" as="xs:integer+"><xsl:sequence select="if(exists(/dryad:*/dcterms:license)) then 1 else 0"></xsl:sequence></xsl:variable><xsl:text>"</xsl:text><xsl:value-of select="'exists'"></xsl:value-of><xsl:text>":"</xsl:text><xsl:value-of select="sum($FGDC_Mandatory.itemExists.d1e5063)"></xsl:value-of><xsl:text>",
</xsl:text><xsl:variable name="FGDC_Mandatory.itemCounts.d1e5063" as="xs:integer+"><xsl:sequence select="count(/dryad:*/dcterms:license)"></xsl:sequence></xsl:variable><xsl:text>"</xsl:text><xsl:value-of select="'count'"></xsl:value-of><xsl:text>":"</xsl:text><xsl:value-of select="sum($FGDC_Mandatory.itemCounts.d1e5063)"></xsl:value-of><xsl:text>",
</xsl:text><xsl:text>
"paths" : [
</xsl:text><xsl:text>
{</xsl:text><xsl:text>"path":"/dryad:*/dcterms:license",</xsl:text><xsl:text>"</xsl:text><xsl:value-of select="'exists'"></xsl:value-of><xsl:text>":"</xsl:text><xsl:value-of select="if(exists(/dryad:*/dcterms:license)) then 1 else 0"></xsl:value-of><xsl:text>",
</xsl:text><xsl:text>"</xsl:text><xsl:value-of select="'count'"></xsl:value-of><xsl:text>"</xsl:text><xsl:text>:</xsl:text><xsl:text>"</xsl:text><xsl:value-of select="count(/dryad:*/dcterms:license)"></xsl:value-of><xsl:text>"
</xsl:text><xsl:text>}</xsl:text><xsl:text>],
...
<xsl:text>{
</xsl:text><xsl:text>"name":"Keyword",</xsl:text><xsl:variable name="FGDC_Mandatory.itemExists.d1e12614" as="xs:integer+"><xsl:sequence select="if(exists(/dryad:*/dcterms:subject)) then 1 else 0"></xsl:sequence></xsl:variable><xsl:text>"</xsl:text><xsl:value-of select="'exists'"></xsl:value-of><xsl:text>":"</xsl:text><xsl:value-of select="sum($FGDC_Mandatory.itemExists.d1e12614)"></xsl:value-of><xsl:text>",
</xsl:text><xsl:variable name="FGDC_Mandatory.itemCounts.d1e12614" as="xs:integer+"><xsl:sequence select="count(/dryad:*/dcterms:subject)"></xsl:sequence></xsl:variable><xsl:text>"</xsl:text><xsl:value-of select="'count'"></xsl:value-of><xsl:text>":"</xsl:text><xsl:value-of select="sum($FGDC_Mandatory.itemCounts.d1e12614)"></xsl:value-of><xsl:text>",
</xsl:text><xsl:text>
"paths" : [
</xsl:text><xsl:text>
...
</xsl:text><xsl:text>"rubricType":"DCITE",</xsl:text><xsl:text>"rubricVersion":"0.1",</xsl:text><xsl:text>"rubricMax":"40",</xsl:text><xsl:variable name="rubricExists" as="xs:integer+"><xsl:sequence select="$FGDC_Mandatory.itemExists"></xsl:sequence><xsl:sequence select="$FGDC_MandatoryIfApplicable.itemExists"></xsl:sequence></xsl:variable><xsl:text>"</xsl:text><xsl:value-of select="'rubricExists'"></xsl:value-of><xsl:text>":"</xsl:text><xsl:value-of select="sum($rubricExists)"></xsl:value-of><xsl:text>",

...
{</xsl:text><xsl:text>"title":"UMM-Common Required",</xsl:text><xsl:text>"code":"UMM-Common_Required",</xsl:text><xsl:text>"description":"The NASA Unified Metadata Model/Mapping (UMM) includes a profile for elements that are common to all of the UMM Profiles, the UMM-Common Profile. These are the elements required by that profile.",</xsl:text><xsl:text>"maximum":"23",</xsl:text><xsl:variable name="UMM-Common_Required.itemExists" as="xs:integer+"><xsl:sequence select="if(exists(/metadata/metainfo/metd)) then 1 else 0"></xsl:sequence><xsl:sequence select="if(exists(/metadata/metainfo/metfrd)) then 1 else 0"></xsl:sequence><xsl:sequence select="if(exists(/metadata/idinfo/citation/citeinfo/title)) then 1 else 0"></xsl:sequence><xsl:sequence select="if(exists(/metadata/idinfo/descript/abstract)) then 1 else 0"></xsl:sequence><xsl:sequence select="if(exists(/metadata/idinfo/ptcontac)) then 1 else 0"></xsl:sequence><xsl:sequence select="if(exists(/metadata/distinfo/distrib/cntinfo)) then 1 else 0"></xsl:sequence><xsl:sequence select="if(exists(/metadata/idinfo/spdom/bounding)) then 1 else 0"></xsl:sequence><xsl:sequence select="if(exists(/metadata/idinfo/timeperd/timeinfo/rngdates)) then 1 else 0"></xsl:sequence><xsl:sequence select="if(exists(/metadata/idinfo/citation/citeinfo/onlink)) then 1 else 0"></xsl:sequence><xsl:sequence select="if(exists(/metadata/idinfo/browse/browsen)) then 1 else 0"></xsl:sequence></xsl:variable><xsl:text>"</xsl:text><xsl:value-of select="'existTotal'"></xsl:value-of><xsl:text>":"</xsl:text><xsl:value-of select="sum($UMM-Common_Required.itemExists)"></xsl:value-of><xsl:text>",
</xsl:text><xsl:variable name="UMM-Common_Required.itemCounts" as="xs:integer+"><xsl:sequence select="count(/metadata/metainfo/metd)"></xsl:sequence><xsl:sequence select="count(/metadata/metainfo/metfrd)"></xsl:sequence><xsl:sequence select="count(/metadata/idinfo/citation/citeinfo/title)"></xsl:sequence><xsl:sequence select="count(/metadata/idinfo/descript/abstract)"></xsl:sequence><xsl:sequence select="count(/metadata/idinfo/ptcontac)"></xsl:sequence><xsl:sequence select="count(/metadata/distinfo/distrib/cntinfo)"></xsl:sequence><xsl:sequence select="count(/metadata/idinfo/spdom/bounding)"></xsl:sequence><xsl:sequence select="count(/metadata/idinfo/timeperd/timeinfo/rngdates)"></xsl:sequence><xsl:sequence select="count(/metadata/idinfo/citation/citeinfo/onlink)"></xsl:sequence><xsl:sequence select="count(/metadata/idinfo/browse/browsen)"></xsl:sequence></xsl:variable><xsl:text>"</xsl:text><xsl:value-of select="'countTotal'"></xsl:value-of><xsl:text>":"</xsl:text><xsl:value-of select="sum($UMM-Common_Required.itemCounts)"></xsl:value-of><xsl:text>",
</xsl:text><xsl:text>
"items" : [
</xsl:text><xsl:text>{
